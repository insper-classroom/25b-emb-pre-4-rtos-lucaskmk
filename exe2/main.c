...
SemaphoreHandle_t xSemaphore_g;//criacao do semaphore
...
void btn_2_task(void *p) {
  ...
  while (true) {
    if (!gpio_get(BTN_PIN_G)) {
      ...
      xSemaphoreGive(xSemaphore_g);//da ok no semaphpro
    }
  }
}
void led_2_task(void *p) {
  ...
  while (true) {
    ...
    if (xSemaphoreTake(xSemaphore_g, pdMS_TO_TICKS(500)) == pdTRUE) {//se deu ok roda uma vez
